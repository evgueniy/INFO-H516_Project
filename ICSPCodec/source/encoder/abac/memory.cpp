...
uint32 aligned_bit_copy(uint8 *dest, uint32 dest_bit_offset, uint8 *source, uint32 source_bit_offset, uint32 copy_bit_count) 
{
    ...
    // Divide by 8 to get values in bytes
    ...
    // Wouldn’t there be a problem if copy_bit_count is not a multiple of 8?
    // Say 9 bits are to be copied, but in the end only 8 end up at the dest.
    // -> No, trailing bits are handled in calling code
