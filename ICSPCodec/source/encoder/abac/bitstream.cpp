...
// Is is emptied before the resize.
uint32 bitstream::resize_capacity(uint32 size_in_bits) 
{
    ...
    // To have a multiple of 8, right above
...
// The read_index cannot be higher than the write_index so this is essentially
// deleting the buffer.
...
// As it’s a stream, is_empty is basically that there is nothing more to read
// to reach the write_index, not that there is nothing before for example.
...
evx_status bitstream::read_bit(void *data) 
{
    ...
    *dest &= 0xFE; // 1111 1110
    *dest |= ((data_store[source_byte]) >> source_bit) & 0x1; // Fills last bit
    read_index++;
