...
const int OFFSET = 2048; // pour mapper [-2048, +2047] -> [0, 4095]
const int SYMBOL_COUNT = 4096; // nombre total de symboles
...
struct HuffNode {
    ...
    HuffNode(int s, int f) : symbol(s), freq(f), left(nullptr), right(nullptr) {}
};
...
void makebitstream(FrameData* frames, int nframes, int height, int width, int QstepDC, int QstepAC, int intraPeriod, int predmode, Statistics *stats)
{
	...
	else if(predmode==INTER)
	{
		...
		for(int n=0; n<nframes; n++)
		{
			if(n%intraPeriod==0)
			{
				intraBody(frames[n], tempFrame, cntbits, stats);
				//cout << "intra frame bits: " << cntbits << endl;
			}
			else
			{
				interBody(frames[n], tempFrame, cntbits, stats);
...
void intraBody(FrameData& frm, unsigned char* tempFrame, int& cntbits, Statistics *stats)
{
	int totalblck = frm.nblocks16;
...
void interBody(FrameData& frm, unsigned char* tempFrame, int& cntbits, Statistics *stats)
{
	int totalblck = frm.nblocks16;
	int nblock8   = frm.nblocks8;
	int idx       = 0;
...
void buildHuffmanTable(HuffNode* root, string code, unordered_map<int, string>& table) {
    if (!root) return;
...
int ACentropy(int* reordblck, unsigned char* ACentropyResult) {
    int freq[SYMBOL_COUNT] = {0};

    // Étape 1 : Calcul des fréquences
    for (int i = 1; i <= 63; i++) {
        int symbol = reordblck[i] + OFFSET;
        freq[symbol]++;
    }
    ...
    unordered_map<int, string> huffTable;
    buildHuffmanTable(root, "", huffTable);

    // Étape 4 : Encodage dans un buffer temporaire (std::string)
    string bitstream;
    for (int i = 1; i <= 63; i++) {
        int symbol = reordblck[i] + OFFSET;
        bitstream += huffTable[symbol];
    }

    // Étape 5 : Conversion string binaire -> tableau de unsigned char
    int bitLength = bitstream.length();
    int byteLength = (bitLength + 7) / 8;
    memset(ACentropyResult, 0, byteLength); // buffer supposé assez grand

    for (int i = 0; i < bitLength; i++) {
        int byteIdx = i / 8;
        int bitIdx  = 7 - (i % 8);
        if (bitstream[i] == '1')
            ACentropyResult[byteIdx] |= (1 << bitIdx);
    }

    return bitLength; // retourne le nombre de bits écrits
}

unsigned char* ACentropy(int* reordblck, int& nbits) {
    int freq[SYMBOL_COUNT] = {0};

    // Étape 1 : Comptage des fréquences
    for (int i = 1; i <= 63; i++) {
        int symbol = reordblck[i] + OFFSET;
        freq[symbol]++;
    }
    ...
    // Étape 3 : Génération des codes Huffman
    unordered_map<int, string> huffTable;
    buildHuffmanTable(root, "", huffTable);

    // Étape 4 : Construction du flux binaire encodé
    string bitstream;
    for (int i = 1; i <= 63; i++) {
        int symbol = reordblck[i] + OFFSET;
        bitstream += huffTable[symbol];
    ...
    // Étape 5 : Création du buffer de sortie
    nbits = bitstream.size();
    int byteSize = (nbits + 7) / 8;
    unsigned char* result = (unsigned char*) malloc(byteSize);
    if (!result) {
        cout << "Erreur d'allocation mémoire pour ACentropyResult" << endl;
        exit(1);
    }
    memset(result, 0, byteSize);

    // Remplissage des bits dans le buffer
